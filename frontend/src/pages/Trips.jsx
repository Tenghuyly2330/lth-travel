...
const Trips = () => {
      ...
      useEffect(() => {
            document.title = 'Our Trips | Travel';
            fetchTripsData();
      }, []);
      ...
      const fetchTripsData = async () => {
            try {
                  ...
                  const allTrips = Array.isArray(data) ? [STATIC_UPCOMING_TRIP, ...data] : [STATIC_UPCOMING_TRIP];
                  setTrips(allTrips);
                  setFilteredTrips(allTrips);
            } catch (err) {
                  console.error('Failed to load trips:', err);
                  setError('Unable to load trips list. Please try again.');
            } finally {
                  setLoading(false);
            }
      };
      ...
      useEffect(() => {
            let apiTrips = trips.filter((t) => t.id !== STATIC_UPCOMING_TRIP.id);
            if (searchTerm) {
            ...
            if (selectedCountry !== 'All') {
                  apiTrips = apiTrips.filter((t) => t.country.toLowerCase() === selectedCountry.toLowerCase());
            }
            setFilteredTrips([STATIC_UPCOMING_TRIP, ...apiTrips]);
      }, [searchTerm, selectedCountry, trips]);

      const countries = ['All', ...new Set(trips.map((t) => t.country))];
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
                  ...
                  <div className="bg-white p-4 rounded-3xl border border-stone-200 soft-shadow flex flex-col md:flex-row items-center justify-between gap-4 max-w-4xl mx-auto">
                        <div className="relative w-full md:w-1/2">
                              ...
                              <input
                                    type="text"
                                    placeholder="Search by trip title, city, or country..."
                                    value={searchTerm}
                  ...
                  {loading ? (
                        <Loading message="Fetching trip packages..." />
                  ) : error ? (
                        <ErrorMessage message={error} onRetry={fetchTripsData} />
                  ) : filteredTrips.length === 0 ? (
                        <div className="py-16 text-center space-y-3">
                              <p className="text-slate-500 font-medium">No trips match your current search.</p>
                              <button
                                    onClick={() => {
                                          setSearchTerm('');
